...
public class DrivingRouteOverlay extends RouteOverlay {
    ...
    private void showcolorPolyline(int color) {
    ...
    private void colorWayUpdate(List<TMC> tmcSection) {
        ...
        mPolylineOptionscolor.add(
                new LatLng(
                        tmcSection.get(0).getPolyline().get(0).getLatitude(),
    ...
    private void colorWayUpdate(List<TMC> tmcSection, int color) {
        if (mAMap == null) {
            return;
        }
        if (tmcSection == null || tmcSection.size() <= 0) {
